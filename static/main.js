...
// pick a css value based on the current theme
function themeVar(darkValue, lightValue) {
  return document.body.classList.contains('dark-theme') ? darkValue : lightValue;
}

// build a copy button that copies text from getCodeText() and shows feedback
function makeCopyButton(getCodeText) {
...
function initializeOrgBabelResults() {
  ...
  orgBabelResults.forEach(block => {
    ...
    if (hasPrecedingSrcBlock) {
      ...
      headerElement.style.background = themeVar('var(--dark-purple)', 'var(--light-purple)');
    ...
    codeBlocks.forEach(codeElement => {
      ...
      const copyButton = makeCopyButton(() => codeElement.innerText);
...
function initializeOrgSrcBlocks() {
  ...
  orgSrcBlocks.forEach(block => {
    ...
    block.appendChild(makeCopyButton(() => {
...
function applyOrgBlockThemeStyles(block, typeElement) {
  ...
  // base theme styles
  typeElement.style.background = themeVar('var(--dark-aqua)', 'var(--light-aqua)');
  typeElement.style.color = themeVar('var(--dark-bg0)', 'var(--light-bg0)');

  // type-specific colors (excluding "dummy")
  if (type !== 'dummy') {
    const typeColors = {
      'problem': themeVar('var(--dark-yellow)', 'var(--light-yellow)'),
      'definition': themeVar('var(--dark-green)', 'var(--light-green)'),
      'theorem': themeVar('var(--dark-blue)', 'var(--light-blue)'),
      'proof': themeVar('var(--dark-purple)', 'var(--light-purple)')
    };
    ...
    if (typeColors[type]) {
      ...
      block.style.borderLeftColor = themeVar('var(--dark-aqua)', 'var(--light-aqua)');
    }
  } else {
    // for dummy blocks, use orange color
    block.style.borderLeftColor = themeVar('var(--dark-orange)', 'var(--light-orange)');
  }
}
...
function applyOrgBlockTitleThemeStyles(block, titleElement) {
  ...
  titleElement.style.background = themeVar('var(--dark-bg1)', 'var(--light-bg1)');
  titleElement.style.color = themeVar('var(--dark-fg1)', 'var(--light-fg1)');

  // for dummy blocks, we want a simpler style without borders
  if (type === 'dummy') {
    titleElement.style.background = themeVar('var(--dark-bg2)', 'var(--light-bg2)');
    titleElement.style.color = themeVar('var(--dark-fg2)', 'var(--light-fg2)');
    titleElement.style.borderBottom = 'none';
    titleElement.style.borderRight = 'none';
  } else {
    titleElement.style.borderBottom = themeVar('1px solid var(--dark-bg3)', '1px solid var(--light-bg3)');
    titleElement.style.borderRight = themeVar('1px solid var(--dark-bg3)', '1px solid var(--light-bg3)');
  }
}
